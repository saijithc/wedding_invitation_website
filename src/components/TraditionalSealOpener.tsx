...
export default function TraditionalSealOpener({ onComplete, isOpened }: TraditionalSealOpenerProps) {
  ...
  return (
    <>
      ...
      <div className="w-full max-w-xs mx-auto flex flex-col items-center select-none pt-3 pb-1">

        {/* Subtle label */}
        <div className="flex items-center gap-2 mb-5 w-full justify-center">
          <span className="h-[1px] flex-1 bg-gradient-to-r from-transparent to-[#B89758]/35" />
          <span className="text-[10px] text-[#B89758]/80 tracking-[0.28em] uppercase font-medium">
            {isLit ? 'Opening…' : 'Tap to open'}
          </span>
          <span className="h-[1px] flex-1 bg-gradient-to-l from-transparent to-[#B89758]/35" />
        </div>
        ...
        <button
          ...
          className="group relative flex flex-col items-center gap-4 cursor-pointer disabled:cursor-not-allowed"
        >
          {/* Diya + glow stack */}
          ...
          <div className="text-center">
            <p className={`font-serif font-semibold text-[15px] tracking-wide transition-colors duration-500 ${isLit ? 'text-[#8C6D52]' : 'text-[#3D2A1D] group-hover:text-[#5A4231]'}`}>
              {isOpening ? 'Opening…' : isLit ? 'Lighting the way…' : 'Open Invitation'}
            </p>
            {!isLit && (
              <p className="text-[11px] text-[#8C6D52]/70 tracking-wider mt-0.5 font-medium">
                Tap the diya to reveal
              </p>
            )}
          </div>
        </button>
